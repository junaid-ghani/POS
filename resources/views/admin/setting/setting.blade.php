...
@section('contents')
    <style>
        ...
        .notification_dropdown .select2-container--classic .select2-selection--multiple .select2-selection__choice,
        .notification_dropdown .select2-container--default .select2-selection--multiple .select2-selection__choice,
        .notification_dropdown .select2-container--default .select2-selection--multiple .select2-selection__choice__remove {
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="general_setting-tab" data-bs-toggle="tab"
                            ...
                            role="tab" aria-controls="general_setting-tab-pane" aria-selected="false" tabindex="-1"><i
                                class="fa-solid fa-gear me-2 align-middle d-inline-block"></i>General</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link  " id="business_information_setting-tab" data-bs-toggle="tab"
                            ...
                            aria-controls="business_information_setting-tab-pane" aria-selected="true"><i
                                class="fa-brands fa-slack me-2  align-middle d-inline-block"></i> Company Details</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="receipt_setting-tab" data-bs-toggle="tab"
                            ...
                            role="tab" aria-controls="receipt_setting-tab-pane" aria-selected="false" tabindex="-1"><i
                                class="fa-solid fa-receipt  me-2 align-middle d-inline-block"></i>Receipt Customization</button>
                    </li>
                    <li class="nav-item" role="presentation">
                ...
                <div class="tab-content" id="myTabContent2">
                    ...
                    <div class="setting_tabs tab-pane fade active show" id="general_setting-tab-pane" role="tabpanel"
                        ...
                        <div class="mt-4">
                            <form action="{{ route('admin.setting') }}" method="post" class="mt-4"
                                ...
                                @csrf
                                <input type="hidden" name="setting_id" class="setting_id"
                                ...
                                <div class="form-check form-check-md d-flex align-items-center">
                                    <input class="form-check-input" type="checkbox"
                                        value="{{ $setting->allow_transfer }}" id="allow_transfer" name="allow_transfer"
                                        @if ($setting->allow_transfer == '1') checked @endif>
                                    <label class="form-check-label" for="allow_transfer">Allow Transfers</label>
                                </div>
                                <div class="form-check form-check-md d-flex align-items-center">
                                    <input class="form-check-input" type="checkbox" value="{{ $setting->allow_changes }}"
                                        id="allow_changes" name="allow_changes"
                                        @if ($setting->allow_changes == '1') checked @endif>
                                ...
                                <div class="payment_app" style="width: 50%; margin:20px;">
                                    <div class="form-check-md row mb-3 align-items-center">
                                        <div
                                        ...
                                        <div class="col-4">
                                            <div class="profile-pic-upload mb-0">
                                                <div class="new-employee-field">
                                                    <div class="mb-0">
                                                        <div class="image-upload mb-0" style="width: 165px;">
                                                            <input type="file" name="zella_file">
                                                            <div class="image-uploads">
                                                                <h4><i class="fa-solid fa-arrow-up-from-bracket"></i> Upload QR Code</h4>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="col-4">
                                            {{-- @if (!empty($setting->zella_file))
                                                <img src="{{ asset(config('constants.admin_path') . 'uploads/setting/' . $setting->zella_file) }}" alt="img">
                                            @else
                                                <img src="{{ asset(config('constants.admin_path') . 'img/no_image.jpeg') }}" alt="img">
                                            @endif --}}

                                            <div class="new-logo w-100" >
                                                @if (!empty($setting->zella_file))
                                                    <a href="{{ asset(config('constants.admin_path') . 'uploads/setting/' . $setting->zella_file) }}" target="blank" style="text-decoration:underline; color:#FF9F43;" >                                          
                                                        View Upload QR Code
                                                    </a>
                                                @else
                                                    <a href="{{ asset(config('constants.admin_path') . 'img/no_image.jpeg') }}" target="blank" style="text-decoration:underline; color:#FF9F43;" >
                                                        NO QR Code File Uploaded
                                                    </a>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    <div class="form-check-md row mb-3 align-items-center">
                                        ...
                                        <div class="col-4">
                                            <div class="new-logo w-100">
                                                ...
                                                @if (!empty($setting->paypal_file))
                                                <a href="{{ asset(config('constants.admin_path') . 'uploads/setting/' . $setting->paypal_file) }}" target="blank" style="text-decoration:underline; color:#FF9F43;" >                                          
                                                    View Upload QR Code
                                                </a>
                                                @else
                                                    <a href="{{ asset(config('constants.admin_path') . 'img/no_image.jpeg') }}" target="blank" style="text-decoration:underline; color:#FF9F43;" >
                                    ...
                                    <div class="form-check-md row mb-3 align-items-center">
                                        ...
                                        <div class="col-4">
                                            <div class="new-logo w-100">
                                                ...
                                                @if (!empty($setting->venmo_file))
                                                <a href="{{ asset(config('constants.admin_path') . 'uploads/setting/' . $setting->venmo_file) }}" target="blank" style="text-decoration:underline; color:#FF9F43;" >                                          
                                                    View Upload QR Code
                                                </a>
                                                @else
                                                    <a href="{{ asset(config('constants.admin_path') . 'img/no_image.jpeg') }}" target="blank" style="text-decoration:underline; color:#FF9F43;" >
                                    ...
                                    <div class="form-check-md row mb-3 align-items-center">
                                        ...
                                        <div class="col-4">
                                            <div class="new-logo w-100">
                                                ...
                                                @if (!empty($setting->cash_app_file))
                                                <a href="{{ asset(config('constants.admin_path') . 'uploads/setting/' . $setting->cash_app_file) }}" target="blank" style="text-decoration:underline; color:#FF9F43;" >                                          
                                                    View Upload QR Code
                                                </a>
                                                @else
                                                    <a href="{{ asset(config('constants.admin_path') . 'img/no_image.jpeg') }}" target="blank" style="text-decoration:underline; color:#FF9F43;" >
                                ...
                                <div class="sale" style="width: 25%;">
                                    <label for="" class="form-label">Below Minimum Sales Approver(s)</label>
                                    <select name="sales_approval_request_user[]" class="form-control selectbox_user" id="selectbox_user" multiple>
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="receipt_header">
                                        <div class="card-title">Receipt Header</div>
                                        <div class="form-check form-check-md d-flex align-items-center">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="text-center invoice-bar">
                                                <div class="salesperson">
                                                    <p style="margin: 0 !important; text-align: left;">Salesperson(s):
                                                        <strong>John</strong>
                                                    </p>
                                                </div>
                                                <p style="margin: 0 !important; text-align: left;" >Payment Method(s): <strong> Credit Card</strong> </p>
                                                <br>
                    ...
                    <div class="setting_tabs tab-pane fade" id="notification_setting-tab-pane" role="tabpanel"
                        ...
                        <form action="{{ route('admin.add_notification_item') }}" method="post" class="mt-4">
                            ...
                            <div class="row mt-3 align-items-center notification_header">
                                <div class="col-lg-3">
                                    <h6>Notification</h6>
                                </div>
                                <div class="col-lg-2">
                            ...
                            <div class="notification_append_div">
                                <input type="hidden" id="notification_ids" name="notification_id">
                                @foreach ($notification as $item)
                                    <div class="row mt-3 align-items-center ">
                                        <div class="col-lg-3">
                                            <div class="form-check form-check-md d-flex align-items-center">
                                                <input
                                                    class="form-check-input notification_sale notification_sale_{{ $item->notification_id }}"
                                                    data-notification_id="{{ $item->notification_id }}" value="0"
                                                    @if ($item->status == '1') checked @endif type="checkbox">
                                                <input type="hidden" name="status[]" class="notification_status">
                                                <input type="hidden" name="notification_id_for_status[]"
                                                    value="{{ $item->notification_id }}">
                                                <label
                                                    class="form-check-label notification_row @if ($item->status == '0') notification_info @endif "
                                                    for="notification_sale">{{ $item->name }}</label>
                                                <i style="margin-left: 15px; font-size: 18px; color: #2e99db;"
                                                    class="fa-solid fa-circle-info notification_row @if ($item->status == '0') notification_info @endif"
                                                    data-bs-placement="right" data-bs-toggle="tooltip"
                                                    data-bs-html="true" title=""
                                                    data-bs-original-title="<em>{{ $item->message }}</em>"></i>
                                            </div>
                                        </div>
                                        @php
                                            $inputType =
                                                $item->notification_id == '2' ||
                                                $item->notification_id == '3' ||
                                                $item->notification_id == '4'
                                                    ? 'number'
                                                    : 'text';
                                        @endphp
                                        <div
                                            class="col-lg-2 notification_row @if ($item->status == '0') notification_status @endif ">
                                            <input type="{{ $inputType }}"
                                                class="form-control  @if ($item->notification_id == '3') integer_field @endif @if ($item->notification_id == '2' || $item->notification_id == '4') decimals @endif notification_input @if ($item->status == '0') disabled_input2 @endif @if (($item->notification_id == '1') | ($item->notification_id == '5')) disabled_input @endif notification_input_{{ $item->notification_id }} "
                                                @if (($item->notification_id == '1') | ($item->notification_id == '5')) readonly @endif name="trigger_value[]"
                                                value="{{ $item->trigger_value }}" autocomplete="off">
                                            <div class="invalid-feedback">Please enter a Number greater than 0.</div>
                                            <small class="text-danger sale_error_{{ $item->notification_id }}"
                                                style="display: none;">Field is Required</small>
                                        </div>
                                        <div
                                            class="col-lg-3 notification_row notification_selectbox @if ($item->status == '0') notification_dropdown @endif @if ($item->status == '0') notification_status @endif ">
                                            <select name="user_id_{{ $item->notification_id }}[]"
                                                class="form-control  selectbox selectbox_user_{{ $item->notification_id }}"
                                                multiple=""></select>
                                            <small class="text-danger selectbox_user_error_{{ $item->notification_id }}"
                                                style="display: none;">Field is Required</small>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            <div class="text-end">
                                <button type="submit" id="notification_setting_btn" name="notification_setting"
                                    value="notification_setting" class="btn btn-primary Updated_changes">Save</button>
                            </div>
                        </form>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#general_setting_btn").on("click", function(e) {
                ...
                if ($("#allow_payment_app").is(':checked')) {
                    ...
                    if (!$(".payment_app_checkbox:checked").length) {
                        ...
                        $("#payment_app_error").text("Please select any of the applicable Payment Apps")
                            .show();
                        e.preventDefault();
                    } else {
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        ...
                        response.users.forEach(function(user) {
                            ...
                            selectBox.append('<option ' + selected + '  value="' + user .user_id + '">' + user.user_name + '</option>');
                        });
            ...
            function updateNotification() {
                ...
                let ids = []
                $(".notification_sale").each(function() {

                    if ($(this).is(':checked')) {
                        $(this).parent().parent().parent().find(".notification_row").removeClass(
                            'notification_status notification_info ');
                        $(this).parent().parent().parent().find(".notification_input").removeClass(
                            'disabled_input2');
                        $(this).parent().parent().parent().find(".notification_selectbox").removeClass(
                            'notification_dropdown');
                        $(this).parent().find(".notification_status").val(1);
                        ...
                        $(this).parent().find(".notification_status").val(0);
                        $(this).parent().parent().parent().find(".notification_row").addClass(
                            'notification_status notification_info ');
                        $(this).parent().parent().parent().find(".notification_input").addClass(
                            'disabled_input2');
                        $(this).parent().parent().parent().find(".notification_selectbox").addClass(
                            'notification_dropdown');
                    }
                })
                var NotificationIds = `[${ids.join(",")}]`;

                $('#notification_ids').val(NotificationIds);
                ...
                $(document).find('#notification_setting_btn').click(function(e) {

                    // Input values
                    let large_sale = $(".notification_input_2").val();
                    let long_break = $(".notification_input_3").val();
                    console.log(long_break);

                    let total_location_sales = $(".notification_input_4").val();

                    // Selectbox values
                    let selectbox_user_1 = $(".selectbox_user_1").val();
                    let selectbox_user_2 = $(".selectbox_user_2").val();
                    let selectbox_user_3 = $(".selectbox_user_3").val();
                    let selectbox_user_4 = $(".selectbox_user_4").val();
                    let selectbox_user_5 = $(".selectbox_user_5").val();

                    // Flag for error checking
                    let hasError = false;

                    // Check each field and show error if empty, otherwise hide the error

                    if ($(".notification_sale_1").is(':checked')) {

                        if ($.isEmptyObject(selectbox_user_1)) {
                            $(".selectbox_user_error_1").show();
                            hasError = true;
                        } else {
                            $(".selectbox_user_error_1").hide();
                        }


                    }

                    if ($(".notification_sale_2").is(':checked')) {


                        if ($.isEmptyObject(selectbox_user_2)) {
                            $(".selectbox_user_error_2").show();
                            hasError = true;
                        } else {
                            $(".selectbox_user_error_2").hide();
                        }
                    ...
                    if ($(".notification_sale_3").is(':checked')) {

                        if ($.isEmptyObject(selectbox_user_3)) {
                            $(".selectbox_user_error_3").show();
                            hasError = true;
                        } else {
                            $(".selectbox_user_error_3").hide();
                        }
                    ...
                    if ($(".notification_sale_4").is(':checked')) {

                        if ($.isEmptyObject(selectbox_user_4)) {
                            $(".selectbox_user_error_4").show();
                            hasError = true;
                        } else {
                            $(".selectbox_user_error_4").hide();
                        }


                        if (total_location_sales === "") {
                            $(".sale_error_4").show();
                            hasError = true;
                        } else {
                            $(".sale_error_4").hide();
                    ...
                    if ($(".notification_sale_5").is(':checked')) {

                        if ($.isEmptyObject(selectbox_user_5)) {
                            $(".selectbox_user_error_5").show();
                            hasError = true;
                        } else {
                            $(".selectbox_user_error_5").hide();
                        }
                    }
            ...
            $('.notification_sale').change(function() {
                updateNotification()
            });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 'success') {
                        ...
                        let selectBox = $(document).find('.selectbox');
                        selectBox.empty();
                        response.users.forEach(function(user) {
                            selectBox.append('<option value="' + user.user_id + '">' + user
                                .user_name + '</option>');
                        });
                        ...
                        if (response.notificationItem && response.notificationItem.length > 0) {
                            ...
                            response.notificationItem.forEach(function(item) {
                                ...
                                let userIdsArray = JSON.parse(item.user_ids).map(id => parseInt(
                                    id));

                                let selectedNotificationUser = response.users.filter(user =>
                                    userIdsArray.includes(user.user_id));

                                let selectedNotificationUserIds = selectedNotificationUser.map(
                                    user => user.user_id);
                                ...
                                response.users.forEach(function(user) {
                                    ...
                                    let selected = selectedNotificationUserIds.includes(
                                        user.user_id) ? 'selected' : '';
                                    $(selectBoxClass).append('<option ' + selected +
